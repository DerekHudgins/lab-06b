...
app.get('/music', async(req, res) => {
  try {
    const data = await client.query(`SELECT 	m.id, m.name, m.description, c.name as category, m.price, m.owner_id 
    FROM 	  music as m
...
app.get('/music/:id', async(req, res) => {
  try {
    const data = await client.query(`
    SELECT 	m.id, m.name, m.description, c.name as category, m.price, m.owner_id 
    FROM 	  music as m
    JOIN 	  categories as c
    ON 	 	  m.category_id = c.id 
    where m.id=$1;
